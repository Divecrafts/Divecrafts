...
public class NPC {
     ...
     * @param loc The location to spawn it
     */
    ...
    public NPC(String name, Location loc, String skin) {
        try {
            ...
            Bukkit.getOnlinePlayers().forEach(p -> sendPackets(p, TabPacket, SpawnPacket));
            addPlayerConnection();
            addEntityToWorld(loc.getWorld());

            Bukkit.getOnlinePlayers().forEach(p -> sendPackets(p, packetOutEntity, packetHead));
    ...
    public void spawn() {
        ...
        try {
            ...
            Bukkit.getOnlinePlayers().forEach(p -> sendPackets(p, SpawnPacket, TabPacket));
    ...
    public class ProfileLoader {
        ...
        private void addProperties(GameProfile profile) {
            ...
            try {
                ...
                //Mojang api -> 1 request of profile per minute and 600 requests per 10 minutes, may change
            }
        }
